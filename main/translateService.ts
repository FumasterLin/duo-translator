...
export class GoogleTranslateService extends TranslateService {
    ...
    async translateText(
        ...
        // A build without VITE_GOOGLE_API_KEY used to send the literal header
        // `x-goog-api-key: undefined` and surface as an opaque 401. Say the
        // actual problem instead.
        if (!this.apiKey) {
            throw new Error(
                "This build has no Google API key. Set VITE_GOOGLE_API_KEY in .env "
                + "(see .env.example) and rebuild — the endpoint rejects keyless requests.",
            );
        }
